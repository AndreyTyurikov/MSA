...
using (IConnection rabbitConnection = rabbitConnectionFactory.CreateConnection())
{
    ...
    {
        //Объявляем очередь самостоятельно
        ...
        channel.QueueBind(messageQueueName, fanOutExchangeName, "");
        ...
        while (true)
        {
            channel.BasicConsume(messageQueueName, autoAck: true, consumer: channelEventsConsumer);
        }
